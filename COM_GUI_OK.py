...
def toggle_connection():
    ...
    if not is_connected:
        try:
            ...
            is_connected = True
            reading_active = True
            status_label.config(text="Conectado", fg="green") # 'fg' funciona con tk.Label
        ...
        is_connected = False
        status_label.config(text="Desconectado", fg="red") # 'fg' funciona con tk.Label
...
def leer_datos_serial():
    ...
    if reading_active and arduino and arduino.in_waiting > 0:
        try:
            ...
            print(f"Dato no válido recibido: {e}")
...
# ### CAMBIO CLAVE AQUÍ ###
# Se usa tk.Label en lugar de ttk.Label para poder usar la opción 'fg' para el color.
status_label = tk.Label(p1_conexion, text="Desconectado", font=("Helvetica", 12, "bold"), fg="red")
